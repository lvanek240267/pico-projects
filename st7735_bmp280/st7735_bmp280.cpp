...
#define ADDR _u(0x76) // If you leave SDO unconnected, it will be connected to 3.3V (address 0x77)
...
int main(void) 
{
	...
	while (true) 
	{
		...
		myTFT1.TFTdrawText(5, 145, strTempUnit, ST7735_BLUE, ST7735_BLACK, 2);
...
void bmp280_get_calib_paramsHum(struct bmp280_calib_param* params) 
{
	...
	i2c_write_blocking(i2c_default, ADDR, &reg, 1, true);  // true to keep master control of bus
    // read in one go as register addresses auto-increment
    i2c_read_blocking(i2c_default, ADDR, buf, 7, false);  // false, we're done reading
